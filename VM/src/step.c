...
void	do_proceses()
{
	...
	// ft_printf("START do_process: cur_cycle %d | process_cout %d\n", g_vm->cur_cycle, g_vm->process_count);
	while(process)
	{
		// ft_printf("		cur_cycle %d | process_cout %d | process id %d | process_position %d\n", g_vm->cur_cycle, g_vm->process_count, j, process->position);
		if (!process->opcode)
		{
			// ft_printf("		!opcode start with process pos %u\n", process->position);
